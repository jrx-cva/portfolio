...
public class ConstruirJugadores {
    ...
    public void draw(Graphics2D g) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                jugador[i].casillas[j].draw(g, x_inicial[i][j], y_inicial[i][j], i); //initialx= es la posición de la primera ficha de cada jugador en la linea vertical; initialy= es la posicion de la primera ficha en la linea horizontal;
